...
async def food_chosen(message: types.Message, state: FSMContext):
    if message.text.lower() not in available_food_names:
        ...
        return
    await state.update_data(chosen_food=message.text.lower())    # сохранить полученный текст в хранилище данных FSM, это словарь, поэтому воспользуемся функцией update_data() и сохраним текст сообщения под ключом chosen_food и со значением message.text.lower()
